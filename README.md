...
| 418 重试等待 | 1/2 秒；180/300 秒 | 180 秒等待后可以继续推进并完成图书馆网络下的批次，但尚未完成不同等待值的系统对照；当前 180/300 秒仅作临时保守值，仍待优化 |
...
HTTP 418 当前同样使用 180 秒、300 秒两档恢复等待，再原样重试失败页；它不会套用 414 专属的后续请求间隔提升，因为 418 可能发生在账号解析、高级搜索或旧时间线等不同端点。这里的 180/300 秒是当前的保守临时值，尚未完成 418 专门的参数对照，后续仍待优化，不应理解为微博规定的固定阈值。
...
同一批量任务在不同网络下表现差异明显，这次实测提示网络质量可能是触发或放大 418 的因素之一：

- 学校图书馆 Wi‑Fi 实测通常只有约 3–5 Mbps，偶尔低于 1 Mbps，较少时候才能达到约 20 Mbps。`wrs.txt` 的 9 个账号在该网络下仍成功完成（总用时 17 分 45 秒）；大账号旧时间线期间出现过 418，等待 180 秒后可以继续推进并自然耗尽。
- 宿舍手机热点连接较稳定。同类批量任务在该网络下通常只需约 5–6 分钟，明显快于图书馆网络。

这只是一次带有网络环境差异的现场对照，不能据此把 418 归因于带宽本身；微博端的会话状态、请求节奏、目标账号规模和服务端波动也可能同时影响结果。后续做重试参数实验时，应记录网络类型、稳定性和是否连续运行过其他批次；当前 418 的 180/300 秒等待仍标记为待优化。
